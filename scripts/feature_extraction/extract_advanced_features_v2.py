#!/usr/bin/env python3
"""
Advanced Feature Extraction v2 for PSA Card Grading

Adds three key upgrades on top of v1 (6,202 features):
- LoG Kurtosis: scratch detection via distribution outliers
- Corner Circularity: 4π × Area / Perimeter² roundness metric
- High-Resolution Corner Analysis: corners analyzed at original resolution

Total features per sample: 6,298
"""
...
# Configuration (keep v1 defaults to preserve baseline feature count)
IMG_SIZE = 224
LBP_RADIUS = 3
...
def extract_lbp_features(gray_224: np.ndarray, n_bins: int = 26) -> np.ndarray:
    lbp = local_binary_pattern(gray_224, LBP_N_POINTS, LBP_RADIUS, method="uniform")
...
def extract_border_features(gray_224: np.ndarray) -> np.ndarray:
    ...
    features.extend(
        [
            float(np.std(border_means)),
            float(np.max(border_means) - np.min(border_means)),
        ]
    )
    return np.array(features)
...
def extract_corner_circularity_features(gray_orig: np.ndarray) -> np.ndarray:
    """
    v2: Corner circularity at multiple scales (4π × Area / Perimeter²).
    Returns 3 scales × 4 corners = 12 features.
    """
    features: list[float] = []
    h, w = gray_orig.shape
...
def extract_highres_corner_features(gray_orig: np.ndarray) -> np.ndarray:
    """
    v2: High-resolution corner features (original resolution).
    Returns 4 corners × 20 features = 80 features.
    """
    h, w = gray_orig.shape
    ...
    for corner in corners.values():
        ...
        # intensity / saturation-ish proxies (grayscale only)
        ...
        perim_norm = float(perim) / float(4 * cs)  # normalize by square perimeter
        circularity = (4.0 * np.pi * area / (perim**2)) if perim > 1e-9 else 0.0

        # simple texture diffs
...
def extract_all_features_v2(img_bgr_orig: np.ndarray) -> np.ndarray:
    """
    v1 baseline (6,202) + v2 additions (96) = 6,298 total.
    """
    ...
    feats = np.concatenate([baseline, log_kurt, corner_circ, hires_corner])
    return feats


def get_feature_names_v2() -> list[str]:
    hog_len = len(extract_hog_features(np.zeros((IMG_SIZE, IMG_SIZE), dtype=float)))
    names: list[str] = []

    names.extend([f"hog_{i}" for i in range(hog_len)])
    ...
    if len(names) != 6298:
        raise RuntimeError(f"Expected 6,298 feature names, got {len(names)}")
    return names
...
def process_dataset(data_dir: str, output_base: str) -> None:
    ...
    for class_dir in class_dirs:
        ...
        for i, img_path in enumerate(image_files):
            ...
            if img_bgr is None:
                continue
            try:
                feats = extract_all_features_v2(img_bgr)
    ...
    X = np.vstack(all_features) if all_features else np.zeros((0, 6298))
    ...
    # Pickle: keep small, R will use CSV
    import pickle
...
def extract_single(image_path: str, output_csv: str) -> None:
    ...
    feature_names = get_feature_names_v2()
    feats = extract_all_features_v2(img_bgr)
    if feats.shape[0] != 6298:
        raise SystemExit(f"Expected 6,298 features, got {feats.shape[0]}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Extract advanced features v2 (6,298 dims).")
    parser.add_argument("--data-dir", default="data/training", help="Training directory with class folders.")
    parser.add_argument("--output-base", default="models/advanced_features_v2", help="Output base path (no extension).")
    parser.add_argument("--image", default=None, help="Single image path (writes one-row CSV).")
    parser.add_argument("--output-csv", default=None, help="Output CSV path for --image mode.")
    args = parser.parse_args()

    if args.image:
        out = args.output_csv or "models/_single_advanced_features_v2.csv"
