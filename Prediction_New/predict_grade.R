...
# Enhanced with Corner Damage Detection
# ============================================
...
# --- Enhanced Feature Extraction with Corner Analysis ---
extract_features <- function(img_path, crop_slab = TRUE) {
  ...
  # === ENHANCED CORNER ANALYSIS (multi-scale) ===
  for (corner_pct in c(0.10, 0.15, 0.20)) {
    ...
    # Corner consistency (damaged corners differ from others)
  ...
  # === BORDER ANALYSIS ===
  border_size <- 5
  features <- c(features, mean(gray[1:border_size,]), sd(gray[1:border_size,]),
...
# --- Load Models (corner-enhanced) ---
model_11class <- readRDS("models/psa_rf_corners.rds")
model_3class <- readRDS("models/psa_rf_3class_corners.rds")
